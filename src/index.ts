...
function startServer(crashed?: boolean) {
  ...
  const memoryAllocation = process.env.MEMORY_ALLOCATION != undefined ? parseInt(process.env.MEMORY_ALLOCATION) : 1024

  proc = spawn(
    'java',
    [`-Xmx${memoryAllocation}M`, `-Xms${memoryAllocation}M`, '-jar', 'server.jar', 'nogui'],
  ...
  proc.stdout?.on('data', (data) => {
    ...
    if (data.includes('Done')) {
      ...
      const autosaveInterval = process.env.AUTOSAVE_INTERVAL
        ...
        : 0;
      const backupInterval = process.env.BACKUP_INTERVAL ? parseFloat(process.env.BACKUP_INTERVAL) : 0
      ...
      if (backupInterval > 0) {
        console.log(`Backing up every ${backupInterval} minutes (${backupInterval / 60} hours)`);
        logs += `HOST: Backing up every ${backupInterval} minutes (${backupInterval / 60} hours)\n`;
        backup = setInterval(() => {
          sendCmd("save-all")
          createBackup()
